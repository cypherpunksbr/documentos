...
Infelizmente, nosso mundo está ao contrário. Você fica rico fazendo coisas ruins e vai para a prisão por fazer coisas boas. Felizmente, graças ao trabalho árduo de pessoas como o projeto Tor[^19], você pode evitar ir para a cadeia tomando algumas precauções simples:

1) Criptografe seu disco rígido[^20]
...
2) Use uma máquina virtual com todo o tráfego roteado pelo Tor
   ...
   Você pode usar projetos como Whonix[^21], Tails[^22], Qubes TorVM[^23] ou algo customizado[^24]. Aqui está[^25] uma comparação detalhada.
